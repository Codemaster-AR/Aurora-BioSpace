...
def ensure_node_modules(app_dir):
    ...
    # Get absolute paths to node/npm
    if platform.system().lower() == "windows":
        node_bin = os.path.join(app_dir, NODE_DIR_NAME, "node.exe")
        npm_bin = os.path.join(app_dir, NODE_DIR_NAME, "npm.cmd")
    else:
        node_bin = os.path.join(app_dir, NODE_DIR_NAME, "bin", "node")
        npm_bin = os.path.join(app_dir, NODE_DIR_NAME, "bin", "npm")
    
    # Force reinstall if node_modules exists to ensure clean start with new node
    node_modules_path = os.path.join(app_dir, "node_modules")
    if os.path.exists(node_modules_path):
        console.print(Panel("[bold yellow]🧹 Cleaning old node_modules...[/bold yellow]", border_style="yellow"))
    ...
    with console.status("[bold cyan]Running 'npm install'... This might take a moment...[/bold cyan]", spinner="dots"):
        try:
            ...
            if result.returncode == 0:
                ...
                # Manually trigger electron installation to ensure binary is downloaded
                console.print(Panel("[bold cyan]🔄 Ensuring Electron is correctly installed...[/bold cyan]", border_style="cyan"))
                install_js = os.path.join(app_dir, "node_modules", "electron", "install.js")
                subprocess.run(
                    f'"{node_bin}" "{install_js}"', 
                    shell=True
                )
            else:
...
def launch_electron(app_dir):
    ...
    if platform.system().lower() == "windows":
        node_bin = os.path.join(app_dir, NODE_DIR_NAME, "node.exe")
        local_node_path = os.path.join(app_dir, NODE_DIR_NAME)
    else:
        node_bin = os.path.join(app_dir, NODE_DIR_NAME, "bin", "node")
        local_node_path = os.path.join(app_dir, NODE_DIR_NAME, "bin")
    ...
    try:
        ...
        # Setup environment with local node in PATH
        env = os.environ.copy()
        env["PATH"] = f"{local_node_path}{os.pathsep}{env.get('PATH', '')}"
        
        # Use absolute path to node
        electron_process = subprocess.Popen(
            f'"{node_bin}" node_modules/electron/cli.js .', 
            cwd=app_dir, 
            shell=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
